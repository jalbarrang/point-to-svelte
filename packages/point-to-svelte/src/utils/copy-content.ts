...
import type { SvelteGrabEntry } from "../types.js";
import { escapeHtmlText } from "./escape-html-text.js";
import { IS_DEMO } from "./runtime-mode.js";

const SVELTE_GRAB_MIME_TYPE = "application/x-point-to-svelte";
...
export const copyContent = (content: string, options?: CopyContentOptions): boolean => {
  ...
  const svelteGrabMetadata: SvelteGrabMetadata = {
    version: VERSION,
    content,
    entries,
    timestamp: Date.now(),
  ...
  const copyHandler = (event: ClipboardEvent) => {
    ...
    event.clipboardData?.setData(SVELTE_GRAB_MIME_TYPE, JSON.stringify(svelteGrabMetadata));
  };
