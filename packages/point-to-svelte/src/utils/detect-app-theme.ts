...
export const watchAppTheme = (
  host: HTMLElement,
  onChange?: (svelteGrabTheme: AppTheme) => void,
): ThemeWatcherResult => {
  ...
  const applyThemeForFingerprint = (inputsFingerprint: string): AppTheme => {
    ...
    const svelteGrabTheme = invertTheme(appTheme);
    if (svelteGrabTheme !== lastAppliedTheme) {
      lastAppliedTheme = svelteGrabTheme;
      host.setAttribute("data-sg-theme", svelteGrabTheme);
      onChange?.(svelteGrabTheme);
    }
    return svelteGrabTheme;
  };
