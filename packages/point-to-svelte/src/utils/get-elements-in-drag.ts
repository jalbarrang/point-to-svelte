...
import {
  getAdapterSelectionElements,
  resolveAdapterElementAtPoint,
} from "../core/selection-adapters.js";
    ...
    for (
      ...
      for (const candidateElement of elementsAtPoint) {
        if (candidateElement.tagName === "CANVAS") {
          ...
          let endpointAdapterElement = candidateElement;
          if (coordinateIndex === 0) {
            endpointAdapterElement = resolveAdapterElementAtPoint(
              candidateElement,
              sampleCoordinates[coordinateIndex],
              sampleCoordinates[coordinateIndex + 1],
            );
          }
          const adapterElements = getAdapterSelectionElements(
            candidateElement,
            endpointAdapterElement,
          );
          if (adapterElements.length > 0 || endpointAdapterElement !== candidateElement) {
            resolvedThreeCanvasElements.add(candidateElement);
            for (const adapterElement of adapterElements) {
              candidates.add(adapterElement);
            }
            if (endpointAdapterElement !== candidateElement) {
              candidates.add(endpointAdapterElement);
            }
            continue;
          }
        }
