...
const ensureStylesInjected = (): void => {
  ...
  styleElement = createStyleElement("data-point-to-svelte-frozen-styles", FROZEN_STYLES);
};
...
// Animations whose target lives in a shadow root are point-to-svelte's own toolbar/
  ...
  if (styleElementToRemove) {
    try {
      if (globalAnimationFreeze.didUseCssFreeze) {
        ...
        // skipped so point-to-svelte's own toolbar/label animations aren't finished.
...
const applyDocumentAnimationFreeze = (snapshot: GlobalAnimationSnapshot): void => {
  ...
  const animationStyleElement = createStyleElement(
    "data-point-to-svelte-global-freeze",
    "",
