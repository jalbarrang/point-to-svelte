...
import { resolveElementContext, type StackFrame } from "./core/context.js";
...
import { resolveAdapterElementAtPoint } from "./core/selection-adapters.js";

export type { ElementAtPointOptions, ElementBounds } from "./types.js";

export { OpenFileError } from "./errors.js";

export interface SvelteGrabElementContext {
  element: Element;
  snippet: string;
  htmlPreview: string;
  stackString: string;
 ...
 * document roots, point-to-svelte UI, ignored subtrees, and transparent page-covering
 ...
 * point-to-svelte copies to the clipboard, plus structured source location
 * and computed styles.
 *
 * @example
 * const ctx = await getElementContext(document.querySelector('.my-button')!);
 * ctx.snippet;       // formatted text identical to point-to-svelte's clipboard output
 * ctx.componentName; // "SubmitButton"
 * ctx.filePath;      // "/src/components/Button.tsx"
 * ctx.lineNumber;    // 12
 */
export const getElementContext = async (element: Element): Promise<SvelteGrabElementContext> => {
...
export const getElementAtPoint = (
  ...
  for (const element of elements) {
    const resolvedElement = resolveAdapterElementAtPoint(element, clientX, clientY);
...
/**
 * Freezes the page by pausing CSS/JS animations, animation-frame loops and
 * global interactions, and preserving pseudo-states (e.g. :hover, :focus) on the
 * given elements.
 *
 * @example
...
/**
 * Restores normal page behavior by resuming animations, restoring
 * interaction handling, and releasing preserved pseudo-states.
 *
 * @example
 * freeze();
 * // ... capture a snapshot ...
