...
export const PANEL_BACKGROUND = "var(--sg-panel-bg)";
export const PANEL_SHADOW = "var(--sg-shadow)";
...
// Path segments marking app-owned reusable UI directories (shadcn-svelte's
// components/ui, a monorepo packages/ui, headless primitives). A bare `/ui/`
// is deliberately excluded: SvelteKit route folders commonly nest feature code
// under `routes/*/ui/`, so matching any `ui` segment would demote real features.
...
// Below point-to-svelte's own overlays so the toolbar stays clickable, above page
// content so the shield absorbs the page's hover, focus, and click. Page content
// stacked inside point-to-svelte's own range (2147483645 and up) paints over the
...
// are var(--sg-panel-bg) so the overlap is invisible).
...
export const FROZEN_ELEMENT_ATTRIBUTE = "data-point-to-svelte-frozen";
export const SAME_ORIGIN_FRAME_ATTRIBUTE = "data-point-to-svelte-same-origin-frame";
export const HIT_TEST_SHIELD_ATTRIBUTE = "data-point-to-svelte-hit-test-shield";
...
// Marks the overlay's comment input; queried by prompt-mode checks in core.
export const SVELTE_GRAB_INPUT_ATTRIBUTE = "data-point-to-svelte-input";

export const USER_IGNORE_ATTRIBUTE = "data-point-to-svelte-ignore";
