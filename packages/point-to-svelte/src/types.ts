...
export interface Theme {
  ...
  /**
   * The floating toolbar that allows toggling point-to-svelte activation
   */
  toolbar?: {
    /**
...
export interface PluginHooks {
  ...
  onStateChange?: (state: SvelteGrabState) => void | Promise<void>;
...
export interface Plugin {
  ...
  actions?: ContextMenuAction[];
  hooks?: PluginHooks;
  setup?: (api: SvelteGrabAPI, hooks: ActionContextHooks) => PluginConfig | void;
}

export interface Options {
  enabled?: boolean;
  /**
   * Confine point-to-svelte to a single container element instead of the whole page.
  ...
  /**
   * Whether to pause the host app's state updates while point-to-svelte is active.
   * Svelte 5 exposes no public way to pause rendering, so this is currently a
   * no-op kept for API parity with React Grab. Pointer events, CSS/JS
   * animations, animation-frame loops and pseudo-states are still frozen.
   * @default true
   */
  freezeAppUpdates?: boolean;
}
...
declare global {
  interface WindowEventMap {
    "point-to-svelte:element-selected": CustomEvent<ElementSelectedEventDetail>;
  }
}
...
export interface SvelteGrabStackFrame {
...
export interface SvelteGrabEntry {
  tagName?: string;
  componentName?: string;
  content: string;
  commentText?: string;
