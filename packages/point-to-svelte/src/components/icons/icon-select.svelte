<script lang="ts">
  ...
  let { size = 14, class: className, rotationDeg = 0 }: Props = $props();
</script>

<span
  class={cn("inline-flex items-center justify-center will-change-transform", className)}
  style="transform: rotate({rotationDeg}deg); transition-property: transform; transition-duration: {SELECT_ICON_ROTATION_TRANSITION_MS}ms; transition-timing-function: cubic-bezier(0.32, 0.72, 0, 1)"
>
  <svg
