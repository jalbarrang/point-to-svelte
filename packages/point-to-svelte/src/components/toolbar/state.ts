...
const STORAGE_KEY = "point-to-svelte-toolbar-state";
...
export const loadToolbarState = (): ToolbarState | null => {
  ...
  try {
    ...
    console.warn("[point-to-svelte] Failed to load toolbar state from localStorage:", error);
  }
  return null;
};
...
export const saveToolbarState = (state: ToolbarState): void => {
  ...
  // of anyone running point-to-svelte on the same origin.
  if (IS_DEMO) return;
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  } catch (error) {
    console.warn("[point-to-svelte] Failed to save toolbar state to localStorage:", error);
  }
};
