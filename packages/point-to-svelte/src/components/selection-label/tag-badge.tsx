...
export const TagBadge: Component<TagBadgeProps> = (props) => {
  ...
  const renderTagLabel = () => (
    <span class="text-[var(--sg-text-primary)] text-[13px] leading-4 h-fit font-medium overflow-hidden text-ellipsis whitespace-nowrap min-w-0">
      <Show when={props.componentName}>
        <span textContent={props.componentName} />
        <span class="text-[var(--sg-text-secondary)]" textContent={`.${props.tagName}`} />
      </Show>
      <Show when={!props.componentName}>
        <span class="text-[var(--sg-text-primary)]" textContent={props.tagName} />
      </Show>
    </span>
  );
