...
import { SVELTE_GRAB_ATTRIBUTE_NAME } from "../utils/svelte-grab-attribute-name.js";
...
export const init = (rawOptions?: Options): SvelteGrabAPI => {
  ...
  const { enabled: _enabled, container: _container, ...settableOptions } = initialOptions;
  ...
  return createRoot((dispose) => {
    ...
    const notifyElementsSelected = async (elements: Element[]): Promise<void> => {
      ...
      window.dispatchEvent(
        new CustomEvent("point-to-svelte:element-selected", {
          detail: {
    ...
    createEffect(
      on(isActivated, (activated) => {
        if (!activated) return;
        if (!pluginRegistry.store.options.freezeAppUpdates) return;
    ...
    const setCursorOverride = (cursor: string | null) => {
      ...
      if (cursor) {
        if (!cursorStyleElement) {
          ...
          cursorStyleElement.setAttribute("data-point-to-svelte-cursor", "");
    ...
    const blockEnterIfNeeded = (event: KeyboardEvent) => {
      ...
      if (shouldBlockEnter) {
        ...
        if (isEventFromOverlay(event, SVELTE_GRAB_INPUT_ATTRIBUTE)) return false;
    ...
    eventListenerManager.addWindowListener(
      ...
      ignoreRealInput((event: KeyboardEvent) => {
        ...
        const isFromSvelteGrabInput = isEventFromOverlay(event, SVELTE_GRAB_INPUT_ATTRIBUTE);
        if (
          isPromptMode() &&
          isTargetKeyCombination(event, pluginRegistry.store.options) &&
          !event.repeat &&
          !isFromSvelteGrabInput
        ) {
          event.preventDefault();
          event.stopPropagation();
          handleInputCancel();
        ...
        const isFromOverlay =
          isEventFromOverlay(event, "data-point-to-svelte-ignore-events") && !isEnterToActivateInput;

        if (isPromptMode() || isFromOverlay) {
          if (isPromptMode() && !isFromSvelteGrabInput && tryHandleBareKeyShortcut(event)) return;
    ...
    eventListenerManager.addWindowListener(
      ...
      ignoreRealInput((event: KeyboardEvent) => {
        ...
        if (isEventFromOverlay(event, "data-point-to-svelte-ignore-events")) return;
    ...
    eventListenerManager.addWindowListener(
      ...
      ignoreRealInput((event: PointerEvent) => {
        ...
        if (!isDragging() && isEventFromOverlay(event, "data-point-to-svelte-ignore-events")) return;
    ...
    eventListenerManager.addWindowListener(
      ...
      ignoreRealInput((event: MouseEvent) => {
        ...
        const isFromOverlay = isEventFromOverlay(event, "data-point-to-svelte-ignore-events");
    ...
    eventListenerManager.addWindowListener(
      "focusin",
      (event: FocusEvent) => {
        if (isEventFromOverlay(event, SVELTE_GRAB_ATTRIBUTE_NAME)) {
          event.stopPropagation();
        }
      },
      { capture: true },
    ...
    eventListenerManager.addDocumentListener(
      ...
      ignoreRealInput((event: ClipboardEvent) => {
        if (isPromptMode() || isEventFromOverlay(event, "data-point-to-svelte-ignore-events")) {
          return;
        }
        if (isRendererActive()) {
          event.preventDefault();
    ...
    onCleanup(() => {
      ...
      throwCollectedErrors(cleanupErrors, "Disposing point-to-svelte failed");
    });
    ...
    // element being selected; the reactive scope tears the tracker down when the
    // source clears or the root disposes.
    createEffect(() => {
    ...
    if (pluginRegistry.store.theme.enabled) {
      // The renderer is dynamically imported so the Svelte runtime and the
      // overlay components stay out of the SSR bundle.
      void import("../components/renderer.svelte")
        .then(({ default: SvelteGrabRenderer }) => {
          if (disposed) return;
          const rendererProps = {
            selectionVisible,
            selectionBounds,
          ...
          } satisfies SvelteGrabRendererProps;

          const rendererInstance = mount(SvelteGrabRenderer, {
            target: rendererRoot,
            props: rendererProps,
          });

          disposeRenderer = () => {
            void unmount(rendererInstance);
          };
        })
        .catch((error) => {
          console.warn("[point-to-svelte] Failed to load renderer:", error);
        });
    }
