...
const createPluginRegistry = (initialOptions: SettableOptions = {}) => {
  ...
  const preparePluginConfig = (plugin: Plugin, api: SvelteGrabAPI): PluginConfig => {
  ...
  const register = (plugin: Plugin, api: SvelteGrabAPI) => {
    if (isDisposed) return;
  ...
  const hooks = {
    ...
    onStateChange: (state: SvelteGrabState) => callHook("onStateChange", state),
